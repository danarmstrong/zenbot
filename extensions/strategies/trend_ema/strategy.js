...
module.exports = function container (get, set, clear) {
  return {
    ...
    getOptions: function () {
      ...
      this.option('reverse_signals', 'reverse BUY/SELL signals', Boolean, false);
    },
    ...
    onPeriod: function (s, cb) {
      ...
      if (typeof s.period.trend_ema_stddev === 'number') {
        if (s.period.trend_ema_rate > s.period.trend_ema_stddev) {
          ...
          s.trend = 'up';
          s.signal = !s.acted_on_trend ? (s.options.reverse_signals ? 'sell' : 'buy') : null;
          s.cancel_down = false;
        }
          ...
          s.trend = 'down';
          s.signal = !s.acted_on_trend ? (s.options.reverse_signals ? 'buy' : 'sell') : null;
        }
      }
      cb();
    },
